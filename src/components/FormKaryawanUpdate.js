...
export default function FormKaryawanUpdate() {
  const { id } = useParams();
  ...
  const handleGetKaryawanById = async (id) => {
    try {
      ...
      setTanggalLahir(res.data.data.tanggal_lahir.split("T")[0]);
      setAlamat(res.data.data.alamat);
      setAgama(res.data.data.agama);
  ...
  useEffect(() => {
    handleGetKaryawanById(id);
  }, [id]);

  const handleUpdateKaryawan = async (data) => {
    try {
      const res = await api.put(`karyawans/${id}`, data, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
        },
      });
